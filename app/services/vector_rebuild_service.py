...
class VectorRebuildService:
    ...
    async def _perform_vector_rebuild(self, specification_uid: str):
        """
        执行实际的向量重建操作
    ...
    async def handle_classification_change(self, spec_uid: str, classifications: List[Dict[str, Any]]) -> bool:
        ...
        try:
            ...
            # 使用任务管理器调度向量化任务，确保同一规范ID的任务串行执行
            # 并取消任何之前的待执行任务
            await task_manager.schedule_vector_task(
                spec_uid, 
                self._perform_vector_rebuild, 
                spec_uid
            )
    ...
    async def _verify_collections_exist(self, specification_uid: str) -> bool:
        ...
        # # 使用与watch_milvus.py脚本中相同的方式连接Milvus
        # client = MilvusClient(DatabaseConfig.uri)
