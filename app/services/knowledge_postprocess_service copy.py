...
class KnowledgePostprocessService:
        ...
        for category_dir in self.chunks_dir.iterdir():
            if category_dir.is_dir() and category_dir.name != "general_knowledge":
                ...
                async def process_subdirectories(directory_path):
                    ...
                    if markdown_files:
                        ...
                        try:
                            ...
                            # 处理每个匹配的分类
                            for category_info, score in matched_categories:
                                # 格式化分类信息为文本
                ...
                async def process_markdown_files(directory_path, markdown_files, category_info, category_info_text):
                    ...
                    logger.info(f"成功处理分类目录 {directory_path.name} 共 {len(results)} 行文本")
    ...
    def save_vectorized_data(self, data: List[Dict[str, Any]], output_file: str = "./data/processed/vectorized_data.json"):
        """
        保存向量化后的数据到文件
...
async def process_all_chunks_and_insert_to_milvus(specification_uid: str = "default", task_uid: str = None):
    ...
    try:
        ...
        await vector_cleanup_service.cleanup_classification_collections(specification_uid)
        logger.info(f"向量数据库非叶节点数据清理完成: {specification_uid}")
    except Exception as e:
        logger.error(f"清理向量数据库非叶节点数据时出错: {e}", exc_info=True)
